...
namespace eng
{
	...
	{
		ImGui::Begin("My Game Engine -- Debug");
		{
			...
			ImGui::ColorEdit3("BackColor", clearColor);

			// 场景调试：比如显示场景物体数量
			ImGui::Text("Object Number: %d", 100); // 替换成你的场景实际数量

			// 调试开关
			ImGui::Checkbox("Enable ImGui", &enabled);
		...
		ImGui::Begin("Cost");
		{
			ImGui::Text("CPU: %.2f%%", 15.6f);
			ImGui::Text("Memory: %.2f MB", 64.5f);
		}
		ImGui::End();
	}
}